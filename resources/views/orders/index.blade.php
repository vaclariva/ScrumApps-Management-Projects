...
@push('blockhead')
    ...
    <link rel="stylesheet" href="{{ asset('assets/css/orders/index.css') }}">
@endpush
@section('content')
    <div class="card">
        <div class="card-body">
            <div class="d-flex flex-column flex-lg-row justify-content-lg-between gap-4 mb-8">
                <a href="{{ route('orders.create') }}" anim="ripple" class="btn tbr_btn tbr_btn--primary d-flex flex-center gap-2">
                ...
                <div class="d-flex flex-column flex-lg-row gap-4">
                    @include('include.default-datatable-search', ['fullInResponsive' => true, 'tableId' => 'table-orders'])
                    @include('orders.partials.filter', ['filterId' => 'filter-orders-id', 'dt' => 'dtOrders'])
                </div>
            </div>
            @include('orders.partials.widget')
            @include('orders.partials.result-filter', ['filterId' => 'filter-orders-id'])
            <div class="overflow-x-auto">
                <div class="text-center text-muted d-none" id="no-data">Tidak ada data yang tersedia dalam tabel</div>
                <table class="tbr_vertical--table" id="table-orders-vertical">
                    <tr data-for='no'>
                        <th>No</th>
                    </tr>
                    <tr data-for='so_number'>
                        <th>Nomor Pesanan</th>
                    </tr>
                    <tr data-for='partner_name'>
                        <th>Pemesan</th>
                    </tr>
                    <tr data-for='product_order_type'>
                        <th>Jenis Produk</th>
                    </tr>
                    <tr data-for='grand_total'>
                        <th>Grand Total</th>
                    </tr>
                    <tr data-for='payment'>
                        <th>Tagihan</th>
                    </tr>
                    <tr data-for='delivery'>
                        <th>Pengiriman</th>
                    </tr>
                    <tr data-for='status'>
                        <th>Status</th>
                    </tr>
                    <tr data-for='user_name'>
                        <th>Pembuat</th>
                    </tr>
                    <tr data-for='ordered_at'>
                        <th>Dibuat</th>
                    </tr>
                    <tr data-for='updated_at'>
                        <th>Diperbarui</th>
                    </tr>
                    <tr data-for='actions'>
                        <th>Aksi</th>
            ...
            <div class="table-responsive">
                @include('include.default-datatable', ['tableId' => 'table-orders', 'columns'=>
                    [
                        'No',
                        'Nomor Pesanan',
                        'Pemesan',
...
@push('blockfoot')
    <script>
        var dtOrders;
        const urlDatatable = '{{ route('orders.list') }}';
        const urlWidget = '{{ route('orders.widget') }}';
        var dataDatatable = function(d){
            ...
            d.type = getCheckboxChecked({name:'type'});
            d.payment = getCheckboxChecked({name:'payment'});
            d.delivery = getCheckboxChecked({name:'delivery'});
            d.status = getCheckboxChecked({name:'status'});
        }
