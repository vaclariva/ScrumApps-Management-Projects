...
@section('content')
    ...
    >
        @csrf
        <h1 class="fw-bolder mb-4">Masuk</h1>
        <p class="mb-10">Masukkan alamat email dan kata sandi.</p>

        @if ($errors->any())
            <div class="tbr_alert tbr_alert--danger text-center mb-10">
                {{ $errors->first() }}
            </div>
        @endif

        @if(Session::has('is_banned'))
            <div class="tbr_alert tbr_alert--danger text-center mb-10">
                {{ Session::get('is_banned') }}
            </div>
        @endif

        @if (isset($_COOKIE['login-status']))
            <div
                class="tbr_alert tbr_alert--danger text-center mb-10"
                id="throttle-alert"
                data-throttle-end="{{ $_COOKIE['throttle-end'] }}"
            >
                Anda telah melakukan 5 kali percobaan gagal.<br/> Silakan tunggu <span class="tbr_text--primary" id="minutes">00</span> menit <span id="seconds" class="tbr_text--primary">00</span> detik.
            </div>
        @endif

        <div class="tbr_alert tbr_alert--danger text-center mb-10 d-none" id="ajax-error"></div>
        ...
        <div class="form-group w-100 mb-5">
            <div class="position-relative">
                <input type="password" name="password" id="password" class="tbr_form form-control" placeholder="Kata Sandi">
        ...
        <div class="d-flex justify-content-between gap-4 w-100 mb-5">
            ...
            <a href="{{ route('password.request') }}" class="tbr_text--primary fw-semibold">Lupa kata sandi?</a>
        </div>
