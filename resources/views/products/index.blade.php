@extends('layouts.app')
@section('pageTitle', 'Daftar Produk')
@section('breadcrumb')
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex flex-column flex-lg-row justify-content-lg-between gap-4 mb-8">
                <a href="{{ route('products.create') }}" anim="ripple" class="btn tbr_btn tbr_btn--primary d-flex flex-center gap-2">
                ...
                <div class="d-flex flex-column flex-lg-row gap-4">
                    ...
                    @include('products.partials.listing.filter', ['filterId' => 'filter-products-id', 'dt' => 'dtProducts'])
                </div>
            </div>
            @include('products.partials.listing.result-filter', ['filterId' => 'filter-products-id'])
...
@push('blockfoot')
    <script>
        var dtProducts;
        const urlDatatable = '{{ route('products.list') }}'
        var dataDatatable = function(d){
            d.type = getCheckboxChecked({name:'type'});
            d.category = getCheckboxChecked({name:'category'});
            d.visibility = getCheckboxChecked({name:'visibility'});
        }
