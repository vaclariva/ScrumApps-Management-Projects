@extends('layouts.app')
@section('pageTitle', 'Daftar Mitra')
@section('breadcrumb')
    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex flex-column flex-lg-row justify-content-lg-between gap-4 mb-8">
                <a href="{{ route('partners.create') }}" anim="ripple" class="btn tbr_btn tbr_btn--primary d-flex flex-center gap-2">
                ...
                <div class="d-flex flex-column flex-lg-row gap-4">
                    @include('include.default-datatable-search', ['fullInResponsive' => true, 'tableId' => 'table-partners'])
                    @include('partners.partials.filter', ['filterId' => 'filter-partners-id', 'dt' => 'dtPartners'])
                </div>
            </div>
            @include('partners.partials.result-filter', ['filterId' => 'filter-partners-id'])
            @include('include.default-datatable', ['tableId' => 'table-partners', 'columns'=>
                [
                    'No',
                    'Nama',
                    'Kelompok',
...
@push('blockfoot')
    <script>
        var dtPartners;
        const urlDatatable = '{{ route('partners.list') }}'
        var dataDatatable = function(d){
            d.group = getCheckboxChecked({name:'group'});
            d.regency = getCheckboxChecked({name:'regency'});
    ...
    <script src="{{ asset('assets/js/partners/index.js') }}"></script>
@endpush
