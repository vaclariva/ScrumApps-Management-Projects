...
@section('content')
    ...
    <div class="card">
        <div class="card-body ">
            <div
                @class(
                    [
                        "d-flex flex-column flex-lg-row align-items-center gap-4 mb-8",
                        'justify-content-between' => $isSuperAdmin,
                        'justify-content-end' => !$isSuperAdmin
                    ])
                >
                <a href="{{ route('users.create') }}" anim="ripple" @class(["btn tbr_btn tbr_btn--primary d-flex flex-center gap-2 w-100 w-lg-auto", 'd-none' => !$isSuperAdmin])>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <mask id="path-2-inside-1_1326_12102" fill="white">
    ...
    @push('blockfoot')
        ...
        <script>
            var dtUser;
            const urlDatatable = '{{ route('users.list') }}';
            var dataDatatable = function(d){
                d.role = getCheckboxChecked({name:'role'});
            }
            ...
            const canDelete = {{ auth()->user()->isSuperAdmin ? 'true' : 'false' }};
        </script>
